#The Program stores the outtime of car from parking slot into Firebase
...
#variable and data structures declarations
outtime=[]
numberplate= {}
...
#Get Frame function i.e to convert vifeo into frames
def getFrame(sec):
        ...
        if (s4[0] != ''):
            ...
            if (confidence >= 85.0):
                ...
                try:
                    ...
                    if conf > confidence:   # confidence=new value and conf =old value
                        numberplate[s3[1]] = conf
                except KeyError:
                    numberplate[s3[1]] = confidence
    return hasFrames

   # result = firebase.get('/python-sample-ed7f7/users/', '')
...
if __name__ == '__main__':
    ...
    for key in list(numberplate.keys()):
       ...
       for key, val in snapshot.items():
            if (val==plate):
                ref.child(plate).update({'Out Time': t})
                 break
            i+=1